...
class HybridQuantizer:
    ...
    def stage1_fast_layer_profiling(self, dataloader, samples=256):
        ...
        print(f"Measuring baseline FP32 accuracy (Samples capped at 512)...")
        # Internal profiling only needs enough samples for relative ranking
        baseline_acc = self._evaluate_accuracy(dataloader, samples=min(samples, 512))
        ...
        for layer_idx, (layer_name, layer_module) in enumerate(tqdm(layers_to_test, desc="Joint Profiling")):
            ...
            for bits in bit_widths:
                ...
                try:
                    ...
                    # Evaluate with joint quantization (Samples capped at 512)
                    acc_quantized = self._evaluate_accuracy(dataloader, samples=min(samples, 512))
    ...
    def stage2_categorize_and_config(self):
        ...
        # greedy_joint_search already returns {layer: {"weight": bits, "activation": bits}}
        self.final_config = config_bits
        wa_config = config_bits
    ...
    def stage3_selective_granular_refinement(self, dataloader, samples=256):
        ...
        for layer_name in tqdm(tier3_layers, desc="Granular Refinement"):
            ...
            # Get baseline for this layer (Samples capped at 512)
            baseline_acc = self._evaluate_accuracy(dataloader, samples=min(samples, 512))
