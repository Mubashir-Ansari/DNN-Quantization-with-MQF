...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.relu = nn.ReLU()
        self.relu1 = self.relu # Alias for framework mapping
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        out = self.bn1(out)
        out = getattr(self, 'relu', F.relu)(out)
        ...
        out += identity
        out = getattr(self, 'relu2', F.relu)(out)
    ...
    def get_activation_mapping(self):
        ...
        for i, layer_group in enumerate([self.layer1, self.layer2, self.layer3, self.layer4]):
            for j, block in enumerate(layer_group):
                ...
                mapping[f"{prefix}.conv1"] = f"{prefix}.relu"
...
def ResNet18(num_classes=43, in_channels=3):
